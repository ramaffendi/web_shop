...
const addToCart = async (req, res, next) => {
    try {
        ...
        let existingCartItem = await CartItem.findOne({ user: req.user._id, product: foundProduct._id });
...
const hapusProduct = async (req, res, next) => {
    try {
        ...
        let existingCartItem = await CartItem.findOne({ user: req.user._id, product: productId });
        ...
        if (existingCartItem) {
            ...
            return res.status(404).json({ error: true, message: 'Item tidak ditemukan di keranjang' });
        }
    } catch (err) {
        console.log(err);
        next(err);
...
const deleteFromCart = async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({ message: 'Terjadi kesalahan saat menghapus item', error: error.message });
    }
};
...
const updateCartItem = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.status(500).json({ message: 'Terjadi kesalahan saat memperbarui item', error: error.message });
    }
};


module.exports = { update, index, addToCart, deleteFromCart, hapusProduct, updateCartItem };
